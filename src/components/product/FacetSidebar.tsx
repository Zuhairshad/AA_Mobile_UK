...
import { facetGroups, type CategorySlug, type FacetKey } from "../../data/taxonomy"
    ...
    <div className="flex flex-col gap-6">
      ...
      {facetGroups[category]
        ...
        const shown = open
          ...
          : [
              ...selected,
              ...unselected.slice(0, Math.max(0, COLLAPSED_LIMIT - selected.length)),
            ]
        ...
        return (
          <fieldset key={group.key} className="border-0 p-0">
            ...
            <ul className="space-y-1">
              {shown.map((option) => (
                <li key={option.value}>
                  <label className="flex cursor-pointer items-center gap-2 rounded px-1 py-1 text-sm hover:bg-gray-100">
                    <input
                      type="checkbox"
